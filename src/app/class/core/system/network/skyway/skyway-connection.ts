...
export class SkyWayConnection implements Connection {
  ...
  private shouldConnect(peerId: string): boolean {
    if (!this.peer || !this.skyWay || !this.peerId) {
      console.log('connect() is Fail. ID가 할당될 때까지 기다려.');
      return false;
    }
  ...
  private getSkyWayErrorMessage(errType: string): string {
    switch (errType) {
      ...
      case 'list-error': return 'SkyWay listAllPeers API가 Disabled 입니다';
      case 'disconnected': return 'SkyWay의 시그널링 서버에 접속되어 있지 않습니다.';
      case 'socket-error': return 'SkyWay의 시그널링 서버와의 통신에 문제가 발생했습니다.';
      case 'invalid-id': return 'Peer ID가 잘못 되었습니다.';
      case 'unavailable-id': return '그 Peer ID는 이미 사용되고 있습니다.';
      case 'invalid-key': return 'SkyWay API 키가 유효하지 않습니다.';
      case 'invalid-domain': return 'SkyWay API 키에는 현재 도메인이 등록되어 있지 않습니다.';
      case 'authentication': return '인증 오류입니다.';
      case 'server-error': return 'SkyWay의 시그널링 서버와의 접속 중에 문제가 있었습니다. 잠시 후에 재시도 해주세요.';
      case 'sfu-client-not-supported': return '이 클라이언트는 SFU 사용을 지원하지 않습니다. 최신 버전의 Google Chrome을 사용해주세요.';
      case 'peer-unavailable': return 'Peer에 데이터를 송신할 수 없었습니다. Peer ID가 정확한지 확인해주세요.';
      case 'signaling-limited': return '시그널링 횟수가 무상이용범위를 초과하여 모든 기능을 이용하실 수 없습니다.（SkyWay Community Edition만）';
      case 'sfu-limited': return 'SFU 서버의 이용량이 무상 이용범위를 초과하여 SFU의 기능을 이용할 수 없습니다. （SkyWay Community Edition만）';
      case 'turn-limited': return 'TURN 서버의 이용량이 무상 이용범위를 초과하여 TURN의 기능을 이용할 수 없습니다. （SkyWay Community Edition만）\n이 상태에서는 일부 사용자의 연결에 문제가 발생할 수 있습니다. ';
      default: return 'SkyWay와 관련된 알 수 없는 오류가 발생했습니다. ';
    }
  }
}
