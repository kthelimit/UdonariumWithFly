...
export class JukeboxComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    Promise.resolve().then(() => this.modalService.title = this.panelService.title = '주크박스');
  ...
  soundTest(event: Event) {
    ...
    const menu: ContextMenuAction[] = [
      { name: '캐릭터 · 지형', subActions: [
        { name: '캐릭터 이동 시작', action: () => { this.playSETest(PresetSound.piecePick); }},
        { name: '캐릭터를 둔다', action: () => { this.playSETest(PresetSound.piecePut); }},
        { name: '지형의 이동 시작 / 종료', action: () => { this.playSETest(PresetSound.blockPick); }},
      ]},
      { name: '다이스 · 코인', subActions: [
        { name: '다이스 심볼을 갖는다', action: () => { this.playSETest(PresetSound.dicePick); }},
      ...
      { name: '카드 · 카드 더미', subActions: [
        ...
        { name: '카드 · 카드 더미를 갖는다', action: () => { this.playSETest(PresetSound.cardPick); }},
        { name: '카드 · 카드 더미를 둔다', action: () => { this.playSETest(PresetSound.cardPut); }},
        { name: '카드 더미를 섞는다', action: () => { this.playSETest(PresetSound.cardShuffle); }},
      ]},
      { name: '그외', subActions: [
        { name: '고정·잠금／해제', action: () => { this.playSETest(PresetSound.lock); }},
        { name: '낙하／제거／삭제', action: () => { this.playSETest(PresetSound.sweep); }},
        { name: '오브젝트 선택', action: () => { this.playSETest(PresetSound.selectionStart); }},
        { name: '변신！', action: () => { this.playSETest(PresetSound.surprise); }}
      ]}
    ];
    this.contextMenuService.open(position, menu, 'SE 테스트');
  }
