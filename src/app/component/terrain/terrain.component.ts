...
export class TerrainComponent implements OnChanges, OnDestroy, AfterViewInit {
  ...
  private makeSelectionContextMenu(): ContextMenuAction[] {
    ...
    if (this.isSelected) {
      ...
      actions.push(
        {
          name: '선택한 지형', action: null, subActions: [
            {
              name: '전부 고정한다', action: () => {
  ...
  private makeContextMenu(): ContextMenuAction[] {
    ...
    let actions: ContextMenuAction[] = [
      ...
      (this.isLocked ? null : { name: `겹치는 순서 ${this.height === 0 ? '' : ' (평면 지형만)' }`, action: null, subActions: [
        {
          name: '평면 지형의 가장 위에', action: () => {
            if (!this.isLocked) {
      ...
      { name: '경사', action: null, subActions: [
        ...
        ContextMenuSeparator,
        {
          name: `${ this.slopeDirection == SlopeDirection.TOP ? '◉' : '○' } 상(북)`, action: () => {
            this.slopeDirection = SlopeDirection.TOP;
          },
          checkBox: 'radio'
        },
        {
          name: `${ this.slopeDirection == SlopeDirection.BOTTOM ? '◉' : '○' } 하(남)`, action: () => {
            this.slopeDirection = SlopeDirection.BOTTOM;
          },
          checkBox: 'radio'
        },
        {
          name: `${ this.slopeDirection == SlopeDirection.LEFT ? '◉' : '○' } 좌(서)`, action: () => {
            this.slopeDirection = SlopeDirection.LEFT;
          },
          checkBox: 'radio'
        },
        {
          name: `${ this.slopeDirection == SlopeDirection.RIGHT ? '◉' : '○' } 우(동)`, action: () => {
      ...
      { name: '벽의 표시', action: null, subActions: [
        ...
        {
          name: `${ this.hasWall && !this.isSurfaceShading ? '◉' : '○' } 음영 없음`, action: () => {
            this.mode = TerrainViewState.ALL;
      ...
      */
      ...
      ContextMenuSeparator,
      { name: '지형 설정을 편집...', action: () => { this.showDetail(this.terrain); } },
