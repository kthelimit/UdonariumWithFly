...
//  @Input() rotateDeg : string = ''
  ...
  onContextMenu(e: Event) {
    ...
    if (this.selectionService.objects.length) {
      menuArray.push({ name: '여기에 모은다', action: () => this.selectionService.congregate(objectPosition) });
    ...
    menuArray.push(
      {
        name: '영향 그리드의 판정 방법', action: null, 
        subActions: [
          { name: `${this.range.fillType == 0 ? '◉' : '○'} 판정 없음(윤곽 안쪽을 전부 칠한다)`, action: () => { this.range.fillType = 0; }, checkBox: 'radio' },
          ContextMenuSeparator,
          { name: `${this.range.fillType == 1 ? '◉' : '○'} 그리드의 중심을 덮는다`, action: () => { this.range.fillType = 1; }, checkBox: 'radio' },
          { name: `${this.range.fillType == 2 ? '◉' : '○'} 그리드의 일부라도 덮는다`, action: () => { this.range.fillType = 2; }, checkBox: 'radio' },
          { name: `${this.range.fillType == 3 ? '◉' : '○'} 그리드의 반 이상을 덮는다`, action: () => { this.range.fillType = 3; }, checkBox: 'radio' },
          { name: `${this.range.fillType == 4 ? '◉' : '○'} 그리드 전체를 덮는다`, action: () => { this.range.fillType = 4; }, checkBox: 'radio' },
        ]
      }
    );
    menuArray.push(ContextMenuSeparator);
    ...
    if (this.range.type == 'CIRCLE' || this.range.type == 'SQUARE' || this.range.type == 'DIAMOND') {
      let menu: ContextMenuAction[] = this.dockableCharacters.length <= 0
        ? this.followingCharactor ? [] : [{ name: '캐릭터가 없습니다', action: null, disabled: true, center: true }] 
      ...
      //}
      menuArray.push({
          name: '근처의 캐릭터를 따른다', action: null, 
          subActions: menu
        });
      menuArray.push(
        this.range.isExpandByFollowing
        ? {
          name: '☑ 따를 시 크기에 맞춰 확대', action: () => {
...
*/
    ...
    menuArray.push(ContextMenuSeparator);
    menuArray.push(
      { name: '사정・범위를 편집...', action: () => { this.showDetail(this.range); } }
    );
